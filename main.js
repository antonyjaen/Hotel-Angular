...
function createWindow () { //aqui procedemos a crear la ventana
  win = new BrowserWindow({width: 800, height: 800 }) //definimos un alto y ancho en el que s einicializará nuestra aplicación

  // le pasamos como ruta el archivo index que se genera luego de ocmpilar nuestra aplicación de Angular
  win.loadURL(url.format({
    pathname: path.join(__dirname, 'dist/PRIMERA/index.html'), //__dirname, 'src/index.html'esta es la ruta de nuestro index luego de compilar en Angular
    protocol: 'file:',
    slashes: true
  }))
